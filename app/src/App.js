...
class App extends Component {
  ...
  handleSubmit = () => {
    axios
      .post(`http://localhost:9000/api/projects`, this.state.newPost)
  ...
  handleChange = e => {
    ...
    if (e.target.type === "checkbox") {
      this.setState({
        newPost: { ...this.state.newPost, completed: e.target.checked }
      });
    } else {
      this.setState({
        ...
        {this.state.projects.map(project => {
          return (
            ...
            >
              ...
              <small>
                Status: {project.completed ? "Done" : "In Progress"}
              </small>
              <ActionList project={project} id={project.id} />
              <button onClick={() => this.handleDelete(project.id)}>
                Delete this project
              </button>
            </div>
          );
